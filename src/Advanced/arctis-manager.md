...
## Run install script
This following script will prepare the Distrobox container, build the app and set the necessary permissions on the host system. Due to the required access to USB devices, this Distrobox executes applications within containers, mounting `/var`, `/etc` and your `/home` directories in the container itself. Arctis Manager heavily relies on your `/home` directory, thus the container is not really used but for the installation process and is not as self-contained as other Distrobox apps.
